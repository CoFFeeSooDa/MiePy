...
# Path of input file
json_path = './input_json/Demo_wavelength_GScat_interpolate.json'
...
# Path of input file
json_path = './input_json/Demo_wavelength_GScat_interpolate_compare.json'
...
# Main loop
if settings['ModeName'] == 'wavelength':
    sample_pt = inputs['Settings']['grid_pts'] + 1
    # Preallocation
    wavelength_compare = np.zeros(sample_pt, dtype=np.float64)
    k = np.zeros(sample_pt, dtype=np.complex128)
    GF_scat = np.zeros([sample_pt, 3, 3], dtype=np.complex128)
    for ii in range(sample_pt):    
        calc['wavelength'] = (settings['wavelength'])[ii]
        ...
        calc['k0br'] = (settings['k0br'])[ii]
        MP.refresh(calc)
        wavelength_compare[ii] = calc['wavelength']
        # Green's Function Compare
        GF_scat[ii, :, :] = MP.dyadic_greens_function_scattering()
        # Wavenumber in the dielectric medium
        k[ii] = MP.ni[MP.source_dipole.region] * MP.k0
...
if __name__ == "__main__":
    ...
    plt.plot(wavelength_compare, np.imag(electric_field_compare[:, 0]), label='Im_E_compare_x', linestyle='--')
    plt.plot(wavelength_compare, np.imag(electric_field_compare[:, 1]), label='Im_E_compare_y', linestyle='--')
    plt.plot(wavelength_compare, np.imag(electric_field_compare[:, 2]), label='Im_E_compare_z', linestyle='--')
    # plt.legend()
    # plt.show()
